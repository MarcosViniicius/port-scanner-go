...
// scanning for more important ports in ports.go
func ScanAllPorts(c *cli.Context) {
	...
	for i := 0; i < len(CommonPorts); i++ {
		...
		go func(portInfo app.PortInfo) {
			defer wg.Done()
			formattedAddress := fmt.Sprintf("%s:%d", address, portInfo.Port)
			conn, err := dialScan(connectionType, formattedAddress, timeout)
			if err != nil {
				fmt.Println("Port", portInfo.Port, "closed")
				return
			}
			fmt.Println("Port", portInfo.Port, "open")
			conn.Close()
		}(CommonPorts[i])
	}
	wg.Wait()
...
// Helper: establish timed connection using net.DialTimeout
